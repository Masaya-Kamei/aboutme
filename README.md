...
| [ft\_transcendence](https://github.com/Masaya-Kamei/ft_transcendence) | リアルタイムなオンライン対戦ゲーム(Pong)やチャットができるwebアプリ <ul><li>Demo: https://transpong-42-mk.com <br> (シークレットウィンドウ or 別ブラウザ使うと異なるユーザーでログイン可能) </li><li>シングルページアプリケーション</li><li>OAuth2を利用した認証</li><li>websocketを使ってリアルタイム通信</li><li>DB、API設計 (プロフィール、フレンド、ブロック、チャット、対戦記録など)</li><li>ワンタイムパスワードによる二要素認証</li><li>GithubActionsを使ってCI/CD (AWS ECSにデプロイ)</li><li>5人チーム (全員webアプリの開発は初めて)</li></ul> | `TypeScript`,<br>`React`,`NestJS`,`Vite`,<br>`42API`,`REST API`,<br>`JWT`,`Cookie`,<br>`OAuth2`,`2FA`,`CORS`,<br>`File Upload`,`Streaming File`<br>`PostgreSQL`,`Prisma(ORM)`,<br>`Socket.IO`,`canvas`,`Swagger`,<br>`docker compose`,`nginx`,<br>`CI/CD`,`Github　Actions`,<br>`Jest`,`Cypress`,<br>`AWS ECS`,`AWS ECR`,<br>`AWS Route53`,`ACM` |
