...
| [ft_services](https://github.com/Masaya-Kamei/ft_services) | kubernetesを使って複数のマイクロサービスを立ち上げる <ul><li>Dockerイメージはすべて`Alpine Linux`を使用</li><li>minikubeで単一ノードのKubernetes環境を構築</li><li>MetalLBでPodにアクセスを振り分ける</li><li>k8s のPod, Deployment, Serviceなどを使用</li><li>LEMP環境, phpmyadmin, FTPSサーバーを構築</li><li>Telegraf, InfluxDB, Grafanaでメトリクスを収集、可視化</li></ul> | `kubernetes`,`docker`,<br>`minikube`,`MetalLB`,<br>`alpine`,`nginx`,<br>`wordpress`,`mysql`,<br>`phpmyadmin`,`ftps`,<br>`InfluxDB`,`Grafana`,<br>`Telegraf` |
| C++ |
...
| [webserv](https://github.com/Masaya-Kamei/webserv) | HTTP/1.1に準拠したNginxライクなHTTPサーバー <ul><li>I/O(`socket`,`file`) が入出力可能になると発火する<br>イベント駆動型</li><li>シングルスレッド</li><li>対応メソッド: `GET`,`POST`,`DELETE`</li><li>CGI 使用可能</li><li>設定ファイルを読み込み可能</li><li>3人チーム</li></ul> | `C++`,`nginx`,<br>`HTTPリクエスト`,<br>`HTTPレスポンス`,<br>`I/O多重化`,`CGI`,<br>`socket通信`,<br>`Google Test`　|
| webアプリ |
